...
if(isset($_POST["submit"])){
  ...
  if(empty($_POST["code"])){
    $codeErr=" Tên đăng nhập không được bỏ trống !";
  }
    ...
    if(!empty($_POST["username"]) && !empty($_POST["email"]) && !empty($_POST["phone"]) && !empty($_POST["code"]) ){
      $sql = "SELECT * FROM users where id like n'$code'";
      $check_id = getOne($sql);
      if(!empty($check_id)){
        if($check_id["id"] != $id_person){
